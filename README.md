...
使用 .exe 版本時，點擊主畫面的「Check for Updates」按鈕即可檢查是否有新版本。若有新版本，按「Update」即可自動下載、驗證 SHA256 雜湊值並替換執行檔。原始碼版本請透過 Git 拉取最新程式碼。

### Q: 草稿會保留嗎？

會。AI 產生的報告草稿會自動儲存至 `drafts/` 資料夾（JSON 格式），即使關閉程式也不會遺失。下次啟動後可在草稿檢視器中繼續檢視或貼入 RIS。

### Q: 彈出視窗會不會干擾批次處理？

不會。程式內建 PopupGuard 功能，會自動偵測並關閉 RIS 彈出的提醒視窗（如「注意」對話框、「無更多待處理檢查」等），不會中斷批次流程。
